...
(grounding and constraint ablation) across three; E3 remains unpopulated
...
\midrule
qwen3.5:4b          & 72.7\% & 93.2\% &  4.8 &  8.6 & 0 \\
gemma3:12b           & 70.5\% & 72.7\% &  3.7 &  7.4 & 0 \\
qwen2.5:14b          & 75.0\% & 81.8\% &  9.3 & 20.1 & 0 \\
\textbf{qwen3.5-9b}  & \textbf{81.8\%} & \textbf{95.5\%} &  8.2 & 21.0 & 0 \\
mistral-small3.2:24b     & 79.5\% & 81.8\% & 29.9 & 52.1 & 0 \\
\bottomrule
\end{tabular}
\end{table}
...
(93.2\%)---the gap between its pass@1 and pass@5 is the largest in the table,
...
mistral-small3.2:24b show almost no such recovery (70.5\%$\to$72.7\% and
79.5\%$\to$81.8\%): their failures are largely the \emph{same} failure on
...
\revised{This row read $\approx$10 for every model in an earlier version of
this table, and carried our argument that escalation judgment is uniformly
hard. Both were an artefact: two of the four escalation cases were unpassable
by construction, because the check accepting ``either \code{needs\_help:} or
\code{clarify:}'' combined two list-returning validators with \code{or},
which reports an error whenever \emph{either} prefix is absent. Rescoring the
stored outputs against the corrected check---no model was called again---moves
159 calls across E1 and E2. What the corrected row shows is close to the
opposite of the original claim: on the underspecified command \emph{form a
triangle}, all five local models and the hosted reference emit the byte-
identical plan \code{\{"type":"idle","reason":"clarify: no group specified
for triangle formation"\}}, i.e.\ they recognise the ambiguity, decline to
act, and say why. What differed was the prefix, and the prompt's own worked
example is the one that teaches \code{clarify:}. Recognition is uniform and
good; only compliance with an undocumented convention was failing.}
...
\code{escalate} ranges 50--100\% pass@1 with no relationship to model size
(qwen2.5:14b, the largest of the mid-size models, is the \emph{worst} on this
category).
\revised{Two models now score 100\% here. The residual failures are of two
...
nor a question, while qwen2.5:14b answers it with a \code{mapf}---it acts on a
command it cannot possibly have resolved. The second is the dangerous one and
is the case that still argues for a check outside the model; the first is
merely malformed. Four cases in this category is too few to say more.}
...
\multirow{4}{*}{\makecell[l]{qwen3.5\\4b}}
 & baseline (E1)   & 72.7\% &        & 93.2\% &        & 16 \\
 & schema on         & 77.3\% & $+4.6$ & 95.5\% & $+2.3$ & 1 \\
 & locations-only     & 75.0\% & $+2.3$ & 90.9\% & $-2.3$ & 11 \\
 & coords-only         & 68.2\% & $-4.5$ & 84.1\% & $-9.1$ & 11 \\
\midrule
\multirow{2}{*}{\makecell[l]{qwen2.5\\14b}}
 & baseline (E1)   & 75.0\% &         & 81.8\% &         & 0 \\
 & schema on         & 72.7\% & $-2.3$  & 81.8\% & $\pm0.0$ & 0 \\
\midrule
\multirow{4}{*}{\makecell[l]{qwen3.5\\9b}}
 & baseline (E1)   & 81.8\% &         & 95.5\% &         & 6 \\
 & schema on         & \textbf{15.9\%} & $\mathbf{-65.9}$ & \textbf{22.7\%} & $\mathbf{-72.8}$ & 3 \\
 & locations-only     & 90.9\% & $+9.1$  & 95.5\% & $\pm0.0$ & 1 \\
 & coords-only         & 90.9\% & $+9.1$  & 95.5\% & $\pm0.0$ & 0 \\
\bottomrule
\end{tabular}
\end{table}
...
same mechanism is catastrophic: pass@1 drops 65.9 points and pass@5 72.8,
...
(95.5\% pass@5 in all three grounding conditions) and \emph{gains} 9.1 points
...
cells produce identical aggregates (90.9\%/95.5\%). These are genuinely
...
70--82\% pass@1 across five locally-hosted 3.4--24B models under a realistic
$\approx$9{,}700-token prompt---a fleet-supervision system built on a single
unguarded plan call would fail roughly one command in three to one in five
...
outside the model rather than trying to prompt it better.
\revised{This no longer follows from the corrected data and should be replaced.
Recognition of ambiguity is in fact uniform and good: every model tested,
including a hosted reference far outside the local size range, declines to act
on an underspecified command and asks the right question. What the data
supports instead is narrower and, we think, more useful: models agree on
\emph{when} to escalate and disagree on \emph{how} to signal it, so the
component worth putting outside the model is the protocol---a deterministic
check that an idle plan carrying a question is routed to the operator
regardless of which prefix the model chose---rather than the judgment itself.
The one genuine judgment failure in the corrected data, a model issuing a
\code{mapf} in response to \emph{repeat the last command}, is a single case
on a single model and cannot carry the original claim.} Fourth, the
