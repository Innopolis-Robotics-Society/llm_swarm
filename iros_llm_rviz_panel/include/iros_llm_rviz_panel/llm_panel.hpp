...
private:
  ...
  // ---- Quick actions -----------------------------------------------------
  void sendHomeForGroup(const RobotGroup & g);
  void sendFormationLineAt(const FormationZone & z, const RobotGroup & g);
  ...
  // ---- Widgets — quick actions strip ------------------------------------
  QWidget     * quick_action_bar_  {nullptr};
  QHBoxLayout * quick_action_row_  {nullptr};
  ...
  // ---- Cached BT state for marker timer (read on GUI thread) ------------
