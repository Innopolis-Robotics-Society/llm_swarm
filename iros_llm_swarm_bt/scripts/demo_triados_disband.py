#!/usr/bin/env python3
"""
demo_triangle.py — 3-robot gather / form-up / disband / move-leader demo.

Drives the REAL stack (Stage + MAPF planner + formation_manager + bt_runner)
through a four-step mission, talking only via the public BT interfaces:
...
Mission:
  ...
  3. Disband   — explicit mode='disband', which runs DisableFormation and
                 deactivates the formation via /formation/deactivate. Followers
                 freeze in place — no manager is tracking them anymore.
  4. MAPF      — move ONLY the leader to point B. Formation is OFF, so the
                 two former followers stay where they were left after step 3.
...
# is used BOTH to pre-position the robots via MAPF (step 1) AND to define
...
# Formation identifier — must match between step 2 (activate) and step 3
# (disband). DisableFormation tells the manager which formation to drop.
FORMATION_ID: str = 'triangle_3'
...
def check_step_status(log, step_name: str, status: str) -> bool:
    """
    Return True if the step should be considered successful.

    OK   — fully nominal, pass silently.
    WARN — degraded but progressing (e.g. formation slightly out of
           tolerance — normal in physical sim), log a warning and pass.
    Everything else (ERROR, HALTED, TIMEOUT) — fail.
    """
    if status == 'OK':
        return True
    if status == 'WARN':
        log.warning(f'{step_name}: completed with WARN — continuing')
        return True
    log.error(f'{step_name}: completion status={status}')
    return False
...
def step1_mapf_triangle_a(runner: DemoRunner) -> bool:
    ...
    if not check_step_status(runner.get_logger(), 'STEP 1', status):
        return False
...
def step2_triangle_formation(runner: DemoRunner) -> bool:
    ...
    runner.get_logger().info(
        f'=== STEP 2: triangle formation {FORMATION_ID}, '
        f'leader robot_{ROBOT_IDS[0]} ===')
    g = LlmCommand.Goal()
    g.mode = 'formation'
    g.reason = 'demo triangle: form up'
    g.formation_id = FORMATION_ID
    ...
    if not check_step_status(runner.get_logger(), 'STEP 2', status):
        return False
...
def step3_disband_formation(runner: DemoRunner) -> bool:
    """
    Explicitly disband the formation.

    Sends mode='disband' with formation_id=FORMATION_ID. The BT's
    DisbandBranch runs DisableFormation, which calls /formation/deactivate
    on the formation_manager. From this point on the followers are no longer
    controlled by the manager — they stop in place near A.
    """
    runner.get_logger().info(
        f'=== STEP 3: disband formation {FORMATION_ID} ===')
    g = LlmCommand.Goal()
    g.mode = 'disband'
    g.reason = 'demo triangle: disband formation'
    g.formation_id = FORMATION_ID

    if not runner.send(g):
        runner.get_logger().error('STEP 3: send failed')
        return False
    status = runner.wait_for_completion('disband', 10.0)
    if not check_step_status(runner.get_logger(), 'STEP 3', status):
        return False
    runner.get_logger().info('=== STEP 3 OK ===')
    return True


def step4_mapf_leader_to_b(runner: DemoRunner) -> bool:
    """
    Move ONLY the leader to point B. Formation is OFF.

    The two former followers are not in robot_ids and no formation_manager
    is tracking them, so they remain where step 3 left them (near A).
    """
    runner.get_logger().info(
        f'=== STEP 4: MAPF leader robot_{ROBOT_IDS[0]} -> B {POINT_B} ===')
    g = LlmCommand.Goal()
    g.mode = 'mapf'
    g.reason = 'demo triangle: move leader to B (formation off)'
    g.robot_ids = [ROBOT_IDS[0]]
    g.goals.append(make_point(*POINT_B))

    if not runner.send(g):
        runner.get_logger().error('STEP 4: send failed')
        return False
    status = runner.wait_for_completion('mapf', 180.0)
    if not check_step_status(runner.get_logger(), 'STEP 4', status):
        return False
...
STEPS = [
    ...
    ('step 3', step3_disband_formation, 1.0),
    ('step 4', step4_mapf_leader_to_b, 0.0),
]
